...
// IMPORTANTE: en Android emulador usar 10.0.2.2. Si esto no funciona en un
// dispositivo real, reemplazar por la IP de la PC en la misma red.
...
export default function App() {
  ...
  async function addPerson() {
    if (!firstName.trim() || !lastName.trim() || !age.trim()||!arrivalDate.trim()) {
      Alert.alert('Faltan datos', 'Completa nombre, apellido, años y fecha');
      return;
    }
  ...
  async function deletePerson(person: Person) {
    Alert.alert(
      'Eliminar enano',
      `¿Quieres borrar a ${person.firstName} ${person.lastName}?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (!response.ok) {
              Alert.alert('Error', 'No se pudo eliminar al enano');
              return;
            }

            Alert.alert('Enano eliminado', `${person.firstName} ${person.lastName} fue removido.`);
            await loadPeople();
          },
        },
      ],
...
const styles = StyleSheet.create({
  ...
  deleteButton: { marginTop: 10, padding: 9, borderWidth: 1, borderColor: '#d06c6c', backgroundColor: '#4a1b1b' },
  deleteButtonText: { color: '#ffd7d7', textAlign: 'center', fontFamily: 'monospace', fontSize: 12, fontWeight: 'bold' },
});
